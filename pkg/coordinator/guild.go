...
import "github.com/giongto35/cloud-game/v2/pkg/comm"
...
func NewGuild() Guild { return Guild{comm.NewNetMap()} }
...
func (g *Guild) findByPingServer(address string) *Worker {
	w, _ := g.FindBy(func(w comm.NetClient) bool {
		worker := w.(*Worker)
...
func (g *Guild) filter(fn func(w *Worker) bool) (l []*Worker) {
	g.ForEach(func(w comm.NetClient) {
		worker := w.(*Worker)
		if fn(worker) {
			l = append(l, worker)
		}
