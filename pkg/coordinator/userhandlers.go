...
func (u *User) HandleWebrtcInit() {
	...
	if err != nil || resp == "" {
		u.Logf("error: webrtc init failed, %v", err)
		return
	}
...
func (u *User) HandleWebrtcAnswer(data json.RawMessage) {
	...
	if err != nil {
		u.Logf("error: broken webrtc answer request %v", err)
		return
	}
...
func (u *User) HandleWebrtcIceCandidate(data json.RawMessage) {
	...
	err := json.Unmarshal(data, &req)
	if err != nil {
		u.Logf("error: broken ICE candidate request %v", err)
		return
	}
...
func (u *User) HandleStartGame(data json.RawMessage, launcher launcher.Launcher) {
	...
	err := json.Unmarshal(data, &req)
	if err != nil {
		u.Logf("error: broken game start request %v", err)
		return
	}
	...
	if req.RoomId != "" {
		...
		if name == "" {
			u.Logf("couldn't decode game name from the room id")
			return
		}
		game = name
	}
	...
	if err != nil {
		u.Logf("couldn't find game info for the game %v", game)
		return
	}
	...
	u.Logf("Received room response from worker: %v", workerResp.Id)

	if err = u.StartGame(); err != nil {
		u.Logf("can't send back start request")
		return
	}
}
...
func (u *User) HandleQuitGame(data json.RawMessage) {
	...
	err := json.Unmarshal(data, &req)
	if err != nil {
		u.Logf("error: broken game quit request %v", err)
		return
	}
...
func (u *User) HandleSaveGame() {
	...
	if err != nil {
		u.Logf("error: broken game save request %v", err)
		return
	}
...
func (u *User) HandleLoadGame() {
	...
	if err != nil {
		u.Logf("error: broken game load request %v", err)
		return
	}
...
func (u *User) HandleChangePlayer(data json.RawMessage) {
	...
	err := json.Unmarshal(data, &req)
	if err != nil {
		u.Logf("error: broken player change request %v", err)
		return
	}
	// TODO: Async
	resp, err := u.Worker.ChangePlayer(u.Id(), u.RoomID, req)
	if err != nil || resp == "error" {
		u.Logf("error: player switch failed for some reason")
	}
	idx, err := strconv.Atoi(resp)
	if err != nil {
		u.Logf("error: broken player change response %v", err)
		return
	}
